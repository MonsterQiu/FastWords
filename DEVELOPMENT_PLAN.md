...
| P1-D | 英英释义（内置离线）+ 例句 + 设置里自定义显示哪些内容 | ✅ 完成 |
| P1-E | Maple Mono 字体 + 音标格式 + 快捷键栏可关闭 | ✅ 完成 |
| P1-F | 统计页：每日复习热力图（GitHub 风格，蓝调）+ 数字统计 | ✅ 完成 |
| P1-G | 升级调度算法：纯 Swift FSRS-6（替代 SM-2） | ✅ 完成 |
| P1-4 | 导入合并 / 去重 / 预览 | ✅ 完成（预览确认 + 拖拽） |
| P1-5 | 系统词典联动 | ✅ 完成（`dict://` + 搜索失败可开） |
| P1-6 | 标准词书格式兼容 | ✅ 完成（Anki tab/HTML、欧路/Front-Back 列名；.apkg 仍未做） |
| P2 | 轮换间隔 / 揭示模式 / 统计 / 多词书 / Keychain | ✅ 大部完成（主动回忆可配、Keychain、统计与多词书已有；全局划词未做） |
